...
#define SERVER_URL "https://github.com/gasp/micro_httpd"
...
get_mime_type(char* name) {
  char* dot;
  dot = strrchr(name, '.');
  if (dot == (char*) 0) return "text/plain; charset=UTF-8";
  ...
  if (strcmp(dot, ".mpeg") == 0 || strcmp(dot, ".mpe") == 0 || strcmp (dot, ".mpg") == 0)
    return "video/mpeg";
  if (strcmp(dot, ".mkv") == 0)
    return "video/mkv";
  if (strcmp(dot, ".mp3") == 0)
